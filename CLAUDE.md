...
**Architectural elegance is the prime objective, not compatibility.** The operator owns every site consuming this package, so a breaking change is a migration he schedules rather than a promise to strangers. That latitude is to be spent: where the right shape and the compatible shape differ, take the right shape, bump the major, and say what moved. It is not licence to churn — a break still has to buy something structural — but "this would break existing sites" is a cost to weigh, never on its own a reason to keep a compromise. Semver stays a real promise for whoever else reads it (see the Git workflow below); it just is not a constraint on the design.
...
- **A file extension is a statement about processing, not a naming habit.** `.hbs` is a template, `.md` is rendered Markdown, `.html` is HTML — and all three are _compiled_, so `{{ }}` in an `.html` partial is interpolated; `.txt` is the one kiss does not compile. An author should be able to predict what happens to a file from its name alone, and anything kiss adds has to keep that promise legible.
...
npx kiss-ssg aikb <script>     # record the site's knowledge base into config.folders.aikb from
                               ...
                               # e.g. from examples/9-migrated-from-v1/: `node ../../bin/kiss-ssg.js aikb router.js`
...
`new Kiss(config)` resolves config, creates a per-instance Handlebars env (with handlebars-layouts) and Remarkable renderer, ensures folders, queues `config.assets.pipeline`'s external commands and then an asset copy (both on one queue, so the steps always run first), registers helpers and partials, and in dev mode starts the server and watcher — plus, per pipeline step that has one, a long-lived `watch` process that `close()` ends. `.page()`/`.pages()`/`.scan()` queue pages: each becomes one caught promise on `_promises` that resolves the model, runs the controller, and pushes a prepared `KissPage` onto `_stack`. Nothing renders until `.generate()`, which waits for `_promises`, renders each stack entry once, awaits the writes, then fires its callback. `.complete()` runs `_settle()` — drain everything (including work queued by callbacks), render whatever the drain left unrendered, repeat until the stack is stable — then **rejects with an `AggregateError`** if any page, controller, callback or the dev server failed, and otherwise resolves (and, under `cleanBuild: 'atomic'`, promotes the staging folder at that point). `.sitemap()` waits for `_promises` and writes `sitemap.xml`; `.llms()` waits on the same queue and writes the llmstxt.org `llms.txt` from the same registry and the same URL join; `.feed()` does the same for an RSS `feed.xml` of the dated pages; `.robots()` writes `robots.txt`, whose `Sitemap:` line is that same join over the sitemap this build actually wrote, and is emitted only when `.sitemap()` was called. Every page carries an `id` (its view's route by default; `<prefix>/<slug>` for a fan-out item), and the `{{link "<id>"}}` helper turns identity into the served path from a live lookup over `_stack` — or into the extension-less canonical path under `config.links.canonical`, which the per-call `canonical=` hash still overrides — failing the render on an id no page claims. `config.links.trailingSlash` decides, for every URL the build emits at once, whether a directory index keeps its slash (`/courses/`, the default and what Netlify serves) or loses it (`/courses`, what Firebase with `trailingSlash: false` serves) — measured, contradictory, and therefore a site's statement about its host rather than a default kiss can be right about. At the start of the settle path, before any staging folder is discarded or promoted, the pages' `aliases` are written as `<build>/redirects.json` — the host-neutral list, always — plus one host file per entry in `config.redirects.format` (unset by default, so a build with aliases and no format writes the IR alone and says so in a warning — only when a page actually carries an alias, so a site with none hears nothing; `['netlify','firebase']` writes both, and a function is a custom writer whose failure fails the build), and every page's extracted `href`/`src` references (`lib/links.js`, gathered at write time beside the page hash) are resolved against the written files and the registry; the report carries `links`, `redirects` (the resolved `rules`, the files written, and the `removed`/`moved`/`collisions` findings against the last record), `feed` and `robots` (including `disallowAll`, because `Disallow: /` removes a site from search silently) beside `aikb`, all advisory. Under `.watch()`, every event goes to `Kiss._handleChange`, which decides between a scoped re-render of matching stack entries (a page-view edit; a partial or layout edit re-renders the pages `lib/dependency-graph.js` recorded as having rendered it, or every page with a notice when it has none) and a whole-site rebuild that replays the pipeline from the logged `_registrations` (`Kiss._replay()`) so edited models and controllers take effect; both kinds go through one serial rebuild queue, and one live reload fires per settled rebuild. Every settled build also assembles one `BuildReport` — `kiss.report()`, `err.report` on the rejection, and a JSON Lines file when `KISS_REPORT` is set — and anything that puts a failure on the list **without** settling a build (a watch asset re-copy, a helpers reload, a scoped page re-render, a callback running after an earlier settle) re-derives that report in place, so the verdict never trails the failure list; the re-derivation runs `buildReport` and nothing else, leaving `_finishBuild()`'s once-per-build side effects (the `KISS_REPORT` line, the `last-build.json` record) per build, and `duration` frozen at the settle rather than growing with the session; with `KISS_CHECK=1` the build is staged and then discarded whether it passed or failed, which is what `kiss-ssg check` runs; with `KISS_AIKB=1` as well, a passing non-dev build also records the site's knowledge base into `folders.aikb`, which is what `kiss-ssg aikb` runs and the only thing that writes that folder. Full detail: `AIKB/kiss.md`, `AIKB/build-report.md`, `AIKB/check.md`, `AIKB/aikb.md`.

## Git workflow

Two automated nets, and they run the same script. A **pre-commit hook** (`.githooks/pre-commit`, activated per-clone by the npm `prepare` script) blocks a commit whose staged files fail prettier — it checks staged blob content, not the working tree, so it is immune to the CRLF noise of a Windows checkout. **CI** (`.github/workflows/ci.yml`) runs `npm run gates` on every push and PR to `main` and `v2`, on **both `ubuntu-latest` and `windows-latest`** — the matrix exists because `lib/pipeline.js` once shipped a defect no Linux run could see, and it means a `skipIf(process.platform !== 'win32')` test is a real gate rather than a note to self, and `prepublishOnly` runs the same gates before `npm publish` can ship anything. Neither replaces the ritual below: CI tells you a branch is broken, the ritual is what sweeps the docs, checks coverage, bumps the version and writes the reflection.
...
- JSDoc is the type source, and it is checked: `npm run typecheck` (a gate) runs `tsc --checkJs` over `lib/`, `scripts/` and `bin/`. A private class field annotated with `@type` needs `@private` in the _same_ JSDoc block — a second comment displaces it and the field lands in the published `types/`, which `test/unit/types.test.js` rejects. **The same displacement applies to a method's own block**: inserting a method between a `/** … @returns */` and the definition it documents orphans it, and the documented method is emitted as `any`. Neither `npm run typecheck` nor the byte-identity check sees that — a fresh emit is wrong in the same way — so `test/unit/types.test.js` pins the signatures that have been lost this way.
...
- URL policy is threaded, not global: `toAbsoluteUrl` (every absolute URL — `{{canonical}}`, `<loc>`, `llms.txt`, the feed, an alias target) and `servedPathFor` (`{{link}}`'s href) must always be handed the **same** `config.links.trailingSlash`, or a page is linked at one URL and canonicalised at another. `toURLKey` is page identity and deliberately takes no policy at all — `isActive` must not depend on where the site is deployed. Do not "unify the three URL functions".
- **Upstream is documented, not engineered around.** Where Node, handlebars, chokidar or sass is wrong or missing something, record it in `AIKB/upstream.md` with the version observed and **how to re-check it**, and live with the consequence. Patching around another project's defect is a commitment to keep patching — the workaround is maintained against every upstream release, it hides the problem from whoever would fix it properly, and the loop costs more than the defect. A cheap local guard is fine; a mechanism is not, and the bar for building one is that the constraint makes a documented promise of kiss's unkeepable. The same file records a **supported** upstream mechanism kiss does not use, because "we hand-rolled it" and "we considered theirs and it does not fit" look identical from the outside and only one is a decision.
- A doc is a claim about the code, not evidence of it — check `lib/` before repeating what `AIKB/`, `llms.txt` or a `planning/plans/` document says about it, and before writing a plan step that depends on it. **The same standard applies to Claude's own claims and to a peer session's**, which is where it keeps failing: say whether a statement is measured or inferred, name a count as coming from a grep rather than asserting it as fact, re-derive a relayed finding locally before acting on it, and when a delegated or independent review did not actually run, say the branch has no independent review rather than letting a self-check stand in for one. **Agreement from the same model family is not confirmation** — a peer session that reaches your conclusion has reproduced your blind spots, not checked your work; it is one opinion held twice. A call carried only by your reasoning plus a peer's assent is still unverified, and saying so is what lets the operator overrule it.
- Say it at the point of decision, not in the reflection: an unanswered operator question, a risk carried by inherited work, a diff that is mostly generated files — surface it before the step that depends on it, not afterwards as a diagnosis. **Where two readings lead to materially different work, that is an `AskUserQuestion`, not a note at the bottom of a report** — a note the operator has to notice and act on has not been raised, it has been filed, and the operator should never have to ask to be asked. An option's **label states the choice** (`Major — 3.0.0`) and the reasoning goes in the description: a label that can be read both ways (`Major — 2.5.0 is wrong`) will be, and the misread arrived after the bump, the changelog entry and the PR had all been made.
