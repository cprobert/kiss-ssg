...
- `buildReport({ stack, failures, manifest, buildDir, stagingDir, mode, startedAt, sitemap, pipeline, llms, aikb })` → `BuildReport`, whose keys are always in this order:
  ...
  - `aikb` — `null` when `.aikb()` was never called, and otherwise `{ folder, written, notes: { missing, dead } }` as `lib/aikb.js` evaluated it (`AIKB/aikb.md`). Appended last, for the same reason. It is passed through **unmapped**: the knowledge base is source-side and committed, so it is never staged and its paths are already the ones a person would type.
...
- **The report is not quite what `AIKB/last-build.json` holds.** That file is `lastBuildRecord(report)` (`lib/aikb.js`): the same keys in the same order, minus `duration` and minus every `pipeline[].duration`, so two identical builds write the same bytes and a committed snapshot only ever changes when the site does. `readReportsFile` and `diffReports` read it happily — neither looks at a timing.
