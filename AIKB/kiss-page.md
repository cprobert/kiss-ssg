...
- `class KissPage`
  ...
  - `async generate()` — compiles the template, renders with `this.options`, injects the livereload script in dev, minifies (skipped in dev), writes the file (and, in dev, the debug `.json`); returns `this.buildTo`. Logs _and rethrows_ render/minify/output-write errors.
...
- A template that fails to _compile_ (or a view file that can't be read) is still log-only: `_getTemplate` returns `null` and `generate()` takes the "Skipping page generation" branch instead of throwing — so a broken view is a skipped page, not a build failure.
