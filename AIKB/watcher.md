...
- A **deleted** page view is the exception to the match-by-view dispatch: it looks like a match (the stack entry still names it), but re-rendering it is exactly wrong — the view is gone, so the render now fails (see `AIKB/kiss-page.md` on `_getTemplate`) and the page's stale output would sit there unrebuilt and unswept. An `unlink` under `pagesDir` therefore always calls `rebuildSite()`. Note what a replay currently does with it: the registration is still replayed, so the page is re-queued, fails again, and `_replay()`'s `complete()` rejects — logged as `Error rebuilding site`. Its `buildTo` is in the new stack, so the orphan sweep does **not** remove the stale output; dropping registrations whose `.hbs` view no longer exists is a separate, unimplemented follow-up (review finding B3).
