...
- A site rebuild is not purely additive: `_replay()` also deletes output files the previous stack wrote that the new stack no longer produces (and re-runs `sitemap()` if one was ever requested), so deleting a model from a `.pages()` fan-out folder, or changing a slug in a controller, actually removes the old page from the dev server — see `AIKB/kiss.md`.
- Neither rebuild callback is awaited by the watcher, but `rebuildPage`'s render *is* tracked on `Kiss._generating`, so a whole-site rebuild triggered moments later waits for it (and for any still-resolving initial build) before it resets the stack — see `AIKB/kiss.md`.
