...
- `_replay()` (watch rebuilds only) discards `_stack`, `_promises` and `_generating`, re-registers partials, then re-runs `page()` for every entry in `_registrations` with `_replaying = true`, and finally `generate()` + `complete()`. Because the whole pipeline re-runs, models are re-read from disk (and URL models are *re-fetched* — a watch rebuild costs a network round trip per remote model) and controllers re-run; `_replaying` also propagates as `fresh` into `applyController`, which busts Node's module caches so an edited controller file reloads (see `AIKB/controller-resolver.md`). The build dir is *not* cleaned by a replay, so output from pages no longer registered would linger. `sitemap()` is not re-run by a replay either (matching v1), so a controller that changes a page's slug in watch mode leaves `sitemap.xml` stale until the next explicit build.
...
- Watcher rebuilds are fire-and-forget from the watcher's point of view: `rebuildPage` calls `page.generate()` without awaiting it, and `rebuildSite` calls `_requestReplay()` without awaiting it.
