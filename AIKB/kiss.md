...
- `.watch({ entry = process.argv[1] } = {})` — starts (once) a `createWatcher()` wired to rebuild pages/site/assets; a site rebuild goes through `_replay()` (below), a page rebuild is a plain re-render; returns `this`.
...
- Watcher rebuilds are fire-and-forget from the watcher's point of view: `rebuildPage` calls `page.generate()` without awaiting it, and `rebuildSite` kicks off `_replay()` with only a `.catch()` attached. A replay *does* track its own work on the fresh `_promises`/`_generating` it installs, but nothing outside `_replay()` awaits that — a rebuild firing while a previous one is still in flight will reset the arrays under it.
