...
- **The cache is bounded at 256 entries, oldest-first.** A file-form key is a path, so entries are bounded by how many stylesheets a site has. A block-form key is the _rendered_ block, and a template is free to interpolate model data into a `{{#sass}}` block, which would mint a unique key per page and grow the cache with the page count. None of the four consuming sites surveyed does that — zero interpolations inside any `{{#sass}}` block — but the engine cannot assume it. For that pathological case eviction degrades to the uncached behaviour, correct but no longer faster; for the ordinary one it never fires.
