...
**The `Sitemap:` line is the reason the module exists.** The crawler blocks above it are boilerplate a site can hand-write and usually does. What a hand-written file cannot do is stay in step with the build — and this repo was its own proof: `examples/_shared/assets/robots.txt` was a static `User-agent: *` / `Allow: /`, copied into six examples, and it never mentioned the sitemap those same examples generate. kiss shipped examples that wrote a sitemap and then failed to advertise it. That is the gap `.llms()` and `.feed()` were built for, one file further out.
