...
- `createAssetManifest()` → `{ record, lookup, toObject, hasOwner, reconcile }`, a fresh manifest with its own `Map`.
...
- **Cleanup follows per-copy inventories.** `lib/assets.js` reconciles each resolved source/target pair after copying. Old hashed names and deleted sources disappear; files another copy still owns remain. The `record` primitive remains available for direct callers.
