...
- **Sass is loaded through `loadSass()` at the point of compile**, not bound at import — see `AIKB/sass.md`. A site with no `.scss` under its assets folder never loads the `sass` package at all.
