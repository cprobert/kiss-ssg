...
- `createPipeline({ steps, logger, dev })` → `{ run, close, watching }`. One per `Kiss` instance, created in the constructor and closed in `close()`.
  ...
  - `close()` → `Promise<void>`: SIGTERM to every watch process's process group, SIGKILL after `KILL_GRACE` (2s), resolving once they have all gone. On Windows, where there are no process groups, both phases are a `taskkill /T /F` of the child's tree instead. A closed pipeline stays closed — a later `run()` runs its steps but starts no watch process.
...
- **Killing the shell is not enough, so a watch process gets its own process group.** `shell: true` means the child is `/bin/sh -c "<command>"`, and a shell that forks rather than execs — dash does, for a command carrying quotes — leaves the real tool running as an orphan _holding the pipes_, so the child's `'close'` never fires: `close()` hung for ever, and a `--watch` compiler outlived the site. Watch commands are therefore spawned `detached` (POSIX), and `signalGroup` signals `-pid`, which reaches the shell and everything it started. Windows has no process groups, and `child.kill()` there reaches only the `cmd.exe` this module spawned — the same orphan, by a different route — so `signalGroup` shells out to `taskkill /pid <pid> /T /F`, which ends the tree. `/F` is not a choice: a console process owning no window cannot be asked to leave, and `child.kill()` was already calling `TerminateProcess`, so nothing gracious is lost. If `taskkill` cannot start, it falls back to the direct kill rather than throwing. **This was a real defect, caught by `test/unit/pipeline.test.js` on Windows only** — the two tests that need a grandchild to die (`really ends the tool…`, `starts it once…`) hung in `close()` and left one orphaned tool per run, while all 19 others passed and every POSIX run was green.
