...
- `status` values and what `Kiss.sitemap()` does with each: `'no-site-url'` returns early *without* invoking the callback (`if (status === 'no-site-url') return`); `'skipped'` and `'written'` both fall through to invoke the callback with `urls` (`null` for `'skipped'`, the array for `'written'`).
