...
export function coveredByAlias(rel, fromSet) {
  ...
  return (
    fromSet.has(toAbsoluteUrl('', toCanonicalPath(rel))) ||
    fromSet.has(servedPathFor(rel))
  )
}
...
export function redirectFindings({
  ...
  for (const page of previous) {
    ...
    moved.push({ id, from: servedPathFor(from), to: servedPathFor(to) })
  }
  ...
  for (const page of previous) {
    ...
    removed.push(servedPathFor(rel))
  }
