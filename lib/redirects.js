...
import { servedPathFor, toAbsoluteUrl, toCanonicalPath } from './utils.js'
...
export function canonicalPathFor(
  ...
  return toAbsoluteUrl('', toCanonicalPath(rel), { trailingSlash })
}
...
export function normaliseAlias(alias) {
  ...
  // The file is space-separated columns and newline-separated rules, so an
  // alias with whitespace inside it would write more than the one line it
  // stands for — and on a fan-out the alias came from the record, which may
  // have been fetched rather than typed. Dropped, like an empty one.
...
export function collectAliases(
  ...
  for (const entry of stack) {
    ...
    const to = canonicalPathFor(entry.buildTo, buildDir, { trailingSlash })
 ...
 */
export function renderRedirectsJson(rules = []) {
  const payload = {
    version: 1,
    rules: sortRules(rules).map((rule) => ({ ...rule, status: 301 })),
  }
 ...
 * @returns {string} pretty-printed JSON, newline-terminated
 */
export function renderFirebaseRedirects(rules = []) {
  const payload = {
    redirects: sortRules(rules).map((rule) => ({
      source: rule.from,
      destination: rule.to,
      type: 301,
    })),
 ...
 * @returns {string} pretty-printed JSON, newline-terminated
 */
export function renderVercelRedirects(rules = []) {
  const payload = {
    redirects: sortRules(rules).map((rule) => ({
      source: rule.from,
      destination: rule.to,
 ...
 */
export function renderHtaccessRedirects(rules = []) {
  const lines = sortRules(rules).map(
    (rule) => `Redirect 301 ${rule.from} ${rule.to}`,
  )
  return lines.length ? `${lines.join('\n')}\n` : ''
}

// The file each built-in format writes beside `redirects.json`, and what goes
// in it. `'none'` is absent on purpose rather than mapped to `null`: the
// dispatch below reads a missing entry as "no host file", so adding a format
// is one row here and nothing else.
//
// **This table is the single source of truth for which format names exist.**
// `lib/config.js` derives `REDIRECT_FORMATS` from its keys rather than
// repeating them, because the two lists drifting apart has a silent failure
// mode and only one direction of it is loud: a name the validator accepts and
// this table does not know passes validation, matches nothing below, writes
// only the IR, and reports success — the exact silent no-op the format block
// was built to abolish, moved from the user's path into the maintainer's.
export const HOST_FORMATS = Object.freeze({
  netlify: { file: '_redirects', render: renderRedirects },
  firebase: {
    file: 'redirects.firebase.json',
    render: renderFirebaseRedirects,
  },
  vercel: { file: 'redirects.vercel.json', render: renderVercelRedirects },
  htaccess: { file: 'redirects.htaccess', render: renderHtaccessRedirects },
})
 ...
 */
export function resolveRedirectFormats(format) {
  ...
  for (const entry of list) {
    if (typeof entry === 'function') {
      // Two references to one function are one writer; two different functions
      // are two, even if they write the same file — that collision is the
      // site's to notice, not this module's to guess at.
      if (seen.has(entry)) continue
      seen.add(entry)
      resolved.push(entry)
      continue
    }
    if (entry === 'none' || entry === undefined || entry === null) continue
    if (seen.has(entry)) continue
    seen.add(entry)
    resolved.push(entry)
  }
 ...
 */
export function normaliseWriterOutput(returned, buildDir) {
  if (returned === undefined || returned === null) return []
  const list = Array.isArray(returned) ? returned : [returned]
  return list.map((entry) => {
    const file = String(entry?.file ?? '')
    const contents = entry?.contents
    if (!file)
      throw new Error(
        'config.redirects.format writer returned an entry with no `file`',
      )
    if (typeof contents !== 'string')
      throw new Error(
        `config.redirects.format writer returned a non-string \`contents\` for ${file}`,
      )
    const normalised = file.replace(/\\/g, '/').replace(/^\/+/, '')
    if (/^[a-zA-Z]:/.test(file) || file.startsWith('/'))
      throw new Error(
        `config.redirects.format writer returned an absolute path (${file}); paths are relative to the build folder`,
      )
    if (normalised.split('/').includes('..'))
      throw new Error(
        `config.redirects.format writer returned a path outside the build folder (${file})`,
      )
    return { file: `${buildDir}/${normalised}`, contents }
  })
}
 ...
 * @returns {boolean}
 */
export function coveredByAlias(rel, fromSet, { trailingSlash = true } = {}) {
  // Either form covers: the canonical one (`/about`), which is how the sitemap
  // and `{{canonical}}` name the page, and the served one (`/about.html`,
  // `/old/`), which is the URL the finding itself reports and the notice tells
  // the operator to paste into `aliases`. A finding that stayed lit after the
  // operator did exactly what it said would teach them to ignore it.
  return (
    fromSet.has(toAbsoluteUrl('', toCanonicalPath(rel), { trailingSlash })) ||
    fromSet.has(servedPathFor(rel, { trailingSlash }))
  )
}
...
export function redirectFindings({
  ...
  for (const page of currentPages) {
    ...
    live.add(rel)
    live.add(canonicalPathFor(page?.buildTo, buildDir, { trailingSlash }))
  ...
  for (const page of previous) {
    ...
    paired.add(from)
    if (from === to) continue
    if (coveredByAlias(from, fromSet, { trailingSlash })) continue
    // Reported as the URLs a browser asked for, not as the files: `/blog/x/`
    // rather than `/blog/x/index.html`. A finding names an address a reader
    // holds a link to, and `aliases` — the fix — are addresses too, so naming
    // the file would be advice nobody can act on. The comparison above stays on
    // the file paths, where it belongs.
    moved.push({
      id,
      from: servedPathFor(from, { trailingSlash }),
      to: servedPathFor(to, { trailingSlash }),
    })
  }
  ...
  for (const page of previous) {
    ...
    if (coveredByAlias(rel, fromSet, { trailingSlash })) continue
    // Served, for the reason `moved` is: this is a URL, not a file.
    removed.push(servedPathFor(rel, { trailingSlash }))
  }
...
export async function writeRedirects(
  stack,
  { config, logger, outputs = undefined, overwrite = true },
) {
  const buildDir = config.folders.build
  const trailingSlash = config.links?.trailingSlash ?? true
  const configured = config.redirects?.format
  const formats = resolveRedirectFormats(configured)
  const rules = collectAliases(stack, { buildDir, trailingSlash })
  const names = formats.map((entry) =>
    typeof entry === 'function' ? 'custom' : entry,
  )
  // An upgrade from a version whose only behaviour was `_redirects` must not
  // lose it in silence. `undefined`/`null` means the site never chose, so a
  // build that has aliases and no host format is told once, by name, what to
  // set. An explicit `'none'` or `[]` is a choice and says nothing.
  const unset = configured === undefined || configured === null
  if (rules.length === 0)
    return { status: 'none', rules, files: [], formats: names, unset: false }

  /** @type {{ file: string, contents: string }[]} */
  const targets = [
    {
      file: `${buildDir}/redirects.json`,
      contents: renderRedirectsJson(rules),
    },
  ]
  for (const entry of formats) {
    if (typeof entry === 'function') {
      // Awaited inside the promise `Kiss._writeRedirects()` catches, so a
      // writer that throws or rejects fails the build exactly as a failed
      // `_redirects` write does. A writer that dies quietly would put us back
      // where this block started: a redirect feature reporting success while
      // the old URLs 404.
      targets.push(
        ...normaliseWriterOutput(
          await entry(rules, { buildDir, config }),
          buildDir,
        ),
      )
      continue
    }
    const host = HOST_FORMATS[entry]
    if (host)
      targets.push({
        file: `${buildDir}/${host.file}`,
        contents: host.render(rules),
      })
  }

  const files = []
  for (const target of targets) {
    if (!overwrite && fs.existsSync(target.file)) {
      logger.info(`Skipping ${target.file}: already exists`)
      continue
    }
    await fs.outputFile(target.file, target.contents)
    outputs?.claim(target.file, 'redirects')
    logger.success(target.file)
    files.push(target.file)
  }
  return {
    status: files.length ? 'written' : 'skipped',
    rules,
    files,
    formats: names,
    unset: unset && names.length === 0,
  }
}
