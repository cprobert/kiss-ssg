...
export async function writeRedirects(
  ...
  const format = config.redirects?.format ?? 'netlify'
  const rules = collectAliases(stack, { buildDir, trailingSlash })
  const name = typeof format === 'function' ? 'custom' : format
  if (rules.length === 0)
    return { status: 'none', rules, files: [], format: name }
  ...
  if (typeof format === 'function') {
    // Awaited inside the try the caller wraps, so a writer that throws or
    // rejects fails the build exactly as a failed `_redirects` write does.
    // A writer that dies quietly would put us back where this block started:
    // a redirect feature reporting success while the old URLs 404.
    targets.push(
      ...normaliseWriterOutput(
        await format(rules, { buildDir, config }),
        buildDir,
      ),
    )
  } else {
    const host = HOST_FORMATS[format]
    if (host)
      targets.push({
