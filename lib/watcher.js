...
export const isInside = (dir) => {
  const root = posix(nodePath.resolve(dir))
  return (p) => {
    const resolved = posix(nodePath.resolve(p))
...
export function createWatcher({
    ...
    watchers.push(
      chokidar
        ...
        .on('all', (event, p) => {
          ...
          logger.notice(`Changed: ${posix(p)}: `)
          helpersChanged(posix(p))
        })
        .on('error', (/** @type {any} */ err) =>
          logger.error('Watcher error', err.message),
        ),
  ...
  const inAssets = isInside(assetsDir)
  ...
  watchers.push(
    chokidar
      .watch(config.folders.src, {
        ignored: (/** @type {string} */ p) => inAssets(p) || inHelpers(p),
        ...WRITE_SETTLE,
      })
      .on('ready', () => {
        scanned = true
