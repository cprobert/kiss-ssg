...
// A file-form key is a path, so the entry count is bounded by how many
// stylesheets a site has — a handful. A block-form key is the *rendered* block,
// and a template is free to interpolate model data into one, which would make
// the key unique per page and grow the cache with the page count. None of the
// four consuming sites surveyed does that (zero interpolations inside any
// `{{#sass}}` block), but the engine cannot assume it, so the cache is bounded.
// Eviction is oldest-first, which for the pathological case degrades to the
// uncached behaviour — correct, just no longer faster — and never fires for the
// ordinary one.
const MAX_ENTRIES = 256

const remember = (key, entry) => {
  compiled.set(key, entry)
  if (compiled.size > MAX_ENTRIES) compiled.delete(compiled.keys().next().value)
}
...
const memoise = (key, compile) => {
  ...
  remember(key, { css: result.css, deps: dependenciesOf(result) })
  return result.css
}
