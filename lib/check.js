...
export const HELP = `kiss-ssg <command> [args…]
 ...
 */
export function describeEngine({ cwd, from = cwd }) {
  let dir = path.resolve(from)
  for (;;) {
    const candidate = path.join(dir, 'node_modules', 'kiss-ssg')
    let stat = null
    try {
    ...
    if (stat) {
      ...
      try {
        ...
        /* a folder with no readable manifest still counts as found */
      }
      // A junction on Windows is a symbolic link to `lstat`, which is what
      // `npm install` makes of a `file:` dependency there.
      const linked = stat.isSymbolicLink()
      return {
        version,
        dir: path.relative(cwd, candidate).replace(/\\/g, '/'),
        linked,
        target: linked ? fs.realpathSync(candidate).replace(/\\/g, '/') : null,
      }
    }
...
export function engineLine(engine) {
  if (!engine)
    return 'kiss-ssg: no node_modules/kiss-ssg found from here — the script resolves the package some other way'
  const from = `kiss-ssg ${engine.version} from ${engine.dir}`
  if (!engine.linked) return from
  // A link says where the files are, not where they came from: pnpm's
  // node_modules/kiss-ssg is a link into node_modules/.pnpm/…, and that IS the
  // registry package (Codex, on the close's review). Only a target outside
  // any node_modules — a working tree — earns the warning.
  return /\/node_modules\//.test(engine.target ?? '')
    ? `${from} — a link into ${engine.target}`
    : `${from} — a link to ${engine.target}, not the registry package`
}
 ...
 * @property {CheckDiffAsset[]} assets emitted assets that changed filename
 */
 ...
 *
 * @typedef {Object} CheckDiffAsset
 * @property {string} source the path a template asks for (`css/site.css`)
 * @property {string} from the file it resolved to in the baseline
 * @property {string} to the file it resolves to now
 */
...
// The manifest keyed by the path templates ask for. A report written before
// assets were recorded simply has none, which reads as an empty map rather
// than as every asset having been removed.
function targetsBySource(report) {
  const targets = new Map()
  for (const asset of report?.assets ?? [])
    if (typeof asset?.source === 'string')
      targets.set(asset.source, asset.target ?? null)
  return targets
}
...
export function diffReports(before = [], after = []) {
  ...
  return (after ?? []).map((report) => {
    ...
    // Only assets both builds emitted: one the baseline never saw has no
    // previous filename to have moved from, and is the asset copy doing its
    // job rather than a fact that explains a page diff.
    const wasAssets = targetsBySource(partners.get(report?.buildDir))
    const assets = []
    for (const [source, to] of targetsBySource(report)) {
      const from = wasAssets.get(source)
      if (wasAssets.has(source) && from !== to)
        assets.push({ source, from, to })
    }
    ...
    return {
      ...
      unchanged,
      assets: assets.sort((a, b) => a.source.localeCompare(b.source)),
    }
  })
}

// A safety valve, not a display limit: only `.css`/`.js` are hashable at all
// (`AIKB/asset-manifest.md`), and under `version` nothing is renamed, so every
// manifest entry records `target === source` and no row can fire. A real site
// moves one or two. The cap costs nothing and bounds a site that bundles per
// page.
const ASSET_LINES = 10
...
export function formatDiff(diff) {
  const moved = diff.assets ?? []
  const pagesMoved =
    diff.added.length + diff.removed.length + diff.changed.length
  const assets = []
  if (pagesMoved > 0 && moved.length > 0) {
    for (const asset of moved.slice(0, ASSET_LINES))
      assets.push(
        `  ~ asset ${asset.source} -> ${asset.to} (was ${asset.from})`,
      )
    if (moved.length > ASSET_LINES)
      assets.push(`  … and ${moved.length - ASSET_LINES} more assets changed`)
  }
  return [
    ...assets,
