...
export function registerHandlebarsHelpers(
  ...
  hbs.registerHelper('asset', function (...args) {
    ...
    if (!emitted) {
      ...
      throw new Error(
        `asset: '${name}' is not in the build${view} — no .copyAssets() emitted it. Check the path, or the folder it should have been copied from.`,
      )
    }
    ...
    // the renaming policy did to it.
    if (hash || !version) return `${emitted}${suffix}`
    ...
    const url = hbs.Utils.escapeExpression(`${emitted}${suffix}`)
