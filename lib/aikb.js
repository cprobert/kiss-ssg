 ...
 * @property {string|null} hash sha1 hex of what the subject *is* — the controller file's bytes with CRLF normalised to LF (so a Windows clone hashes the same), or the step's command — and `null` when there is nothing to hash (a URL model) or nothing to read (a controller file that is not there)
 */
...
function fileReferences(text) {
  ...
  return [...new Set(tokens)].filter(
    (token) =>
      token &&
      // A bare extension is prose about a KIND of file — "one `.json` record
      // per page" — not a reference to one. It can never resolve, so treating
      // it as a reference guarantees a false positive on a sentence anybody
      // might write.
...
function referenceResolver(map, aikbDir) {
  ...
  for (const page of map.pages ?? []) {
    add(page.view)
    add(page.buildTo)
    // A page's **id** is a first-class thing for a note to cite — it is the
    // name the whole site links that page by, and the one the site map's `Id`
    // column exists to list. It has a slash and no extension, so without this
    // it looked exactly like a path to a file that is not there, and a note
    // doing the right thing was reported as dangling.
    add(page.id)
  }
  ...
  return (token) => {
    ...
    // A note is written the way the deployed site reads, so a folder gets
    // cited URL-style: `/docs` is the same folder as the `./docs` the config
    // names, and `posixPath` strips a leading `./` but not a leading `/`.
    // Without this, one slash was the whole difference between a reference
    // that resolved and one reported dangling — on the site's own configured
    // folders, which is the lint at its least credible. The `endsWith`
    // fallback below cannot cover it: it looks for a candidate ending
    // `//docs`, which nothing ever is.
    const unrooted = value.startsWith('/') ? value.slice(1) : null
    if (unrooted && known.has(unrooted)) return true
    ...
    for (const candidate of known) {
      ...
      if (unrooted && candidate.endsWith(`/${unrooted}`)) return true
    }
...
export function evaluateNotes(map, notesDir, { aikbDir = null } = {}) {
  ...
  // `notePathFor` lower-cases the filename it derives, but the rule the
  // generated README states is "the controller's filename without `.js`" — so
  // an author who follows it writes `jobList.md` for `jobList.js` and the
  // engine looks for `joblist.md`. Reporting that as missing AND dead is two
  // findings for one casing difference, and neither is actionable: renaming to
  // satisfy one creates the other.
  //
  // So: tolerant on read, canonical on write. A note is found whatever its
  // case, and a genuinely missing one is still *reported* at the lower-cased
  // path, which stays the single spelling the engine writes and documents.
  // Two notes differing only in case cannot coexist on a case-insensitive
  // filesystem anyway; `present` is sorted, so which one answers is stable.
  const byCase = new Map()
  for (const file of present)
    if (!byCase.has(file.toLowerCase())) byCase.set(file.toLowerCase(), file)
  const onDisk = (file) => byCase.has(file.toLowerCase())
  // The canonical key for a note on disk — what `expected` would call it.
  ...
  for (const file of scanned) {
    ...
    const hash = expected.get(canonical(file))
    if (hash) {
  ...
  return {
    missing: [...expected.keys()].filter((file) => !onDisk(file)).sort(),
    dead: present.filter((file) => !expected.has(canonical(file))).sort(),
    stale: stale.sort(),
