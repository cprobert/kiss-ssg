...
//
// `'netlify'` is the default and is v2.3's behaviour exactly: `_redirects`, the
// Netlify and Cloudflare Pages format. `'firebase'` and `'vercel'` emit a
// **fragment to merge**, never the host's real config file: a site's
...
// `cleanBuild` is, rather than discovered on the deployed site.
/**
 * @param {*} format the merged `redirects.format`
 * @returns {void}
 * @throws if it is neither a known format name nor a writer function
 */
function assertRedirectFormatIsValid(format) {
  if (typeof format === 'function') return
  if (REDIRECT_FORMATS.includes(format)) return
  throw new Error(
    `config.redirects.format must be one of ${REDIRECT_FORMATS.map((name) => `'${name}'`).join(', ')} or a writer function (received ${JSON.stringify(format)})`,
  )
}
