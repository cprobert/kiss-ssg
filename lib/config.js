...
function folderLocations(folder, key) {
  try {
    ...
    const resolved = realFolder(folder)
    const locations = new Set([lexical, resolved])
    // Keep intermediate spellings too: an aliased project parent can hide a
    // source junction located inside output, while its final target is outside.
    // Resolve each parent prefix without following the remaining suffix.
    if (lexical !== resolved) {
      let current = lexical
      const suffix = []
      while (path.dirname(current) !== current) {
        ...
        locations.add(path.join(realFolder(current), ...suffix))
      }
    }
    return [...locations]
  } catch (cause) {
