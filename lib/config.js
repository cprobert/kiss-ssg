...
function folderLocations(folder, key) {
  try {
    ...
    throw new Error(
      `Cannot safely resolve folders.${key} (${folder}): ${cause.message}`,
      {
        cause,
      },
    )
...
// path.relative compares whole segments and honours Windows case semantics.
/** @param {string} parent @param {string} child @returns {boolean} */
function containsFolder(parent, child) {
...
function assertBuildFolderIsSafe(folders) {
  const project = folderLocations(process.cwd(), 'src')
  ...
  for (const key of ['src', ...DERIVED_FROM_SRC, 'helpers', 'aikb']) {
    ...
    if (
      build.some((output) =>
        source.some((input) => containsFolder(output, input)),
      )
    )
      throw new Error(
    ...
    if (
      key !== 'src' &&
      build.some((output) =>
        source.some((input) => containsFolder(input, output)),
      )
    )
      throw new Error(
