...
// What the link scan does. On by default: a site that never sets the block has
// every internal reference in its own output checked, and the finding is
// advisory — it never touches `ok` or the exit code. A block rather than a
// boolean so the ignore list a real deploy eventually wants is a new key here
// rather than a rename of `config.links`.
