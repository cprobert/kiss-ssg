...
export function buildSitemapEntries(
  ...
  return stack
    ...
    .map((entry) => {
      ...
      // The same two functions the `canonical` helper uses, so a page's
      // `<loc>` and its own `<link rel="canonical">` can never disagree — a
      // directory index included, which ends in `/` on both sides.
