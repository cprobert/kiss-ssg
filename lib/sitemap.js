...
import { toAbsoluteUrl, toURLKey } from './utils.js'
...
export function buildSitemapEntries(
  ...
  return stack
    ...
    .map((entry) => {
      ...
      // The same join the `canonical` helper uses, so a page's `<loc>` and its
      // own `<link rel="canonical">` can never disagree.
      const urlPath = toURLKey(entry.buildTo.slice(buildDir.length))
      return {
