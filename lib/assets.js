...
/** Stable identity from source and requested destination (before staging).
 * @param {string} sourceDir
 * @param {string} targetDir
 * @returns {string}
 */
export const assetCopyOwner = (sourceDir, targetDir) =>
  'asset copy: ' +
  JSON.stringify([path.resolve(sourceDir), path.resolve(targetDir)])
 ...
 */
export function compileSassFiles(
  sourceDir,
  targetDir,
  { config, logger, outputs, owner },
) {
  ...
  return [
    ...
    ...all
      ...
      .map(async (sassFile) => {
        ...
        if (outputs && !outputs.canWriteAsset(cssFile, owner))
          return { file, refused: true }
        ...
        try {
          ...
          outputs?.claim(cssFile, owner, 'asset')
          logger.success(cssFile)
          return { file, fingerprint: contentHash(css) }
        } catch (err) {
...
async function recordEmitted(
  sourceDir,
  targetDir,
  { config, manifest, logger, owner, outputs, sass },
) {
  const root = posixPath(sourceDir)
  ...
  const failed = new Set(
    sass.filter((item) => item.error).map((item) => item.file),
  )
  for (const file of globFiles(root, '**/*')) {
    ...
    attempted.add(plain)
    if (outputs && !outputs.canWriteAsset(plain, owner)) continue
    if (failed.has(source)) {
      const previous = manifest.previous(owner, name)
      if (
        previous &&
        (!outputs || outputs.owns(onDisk(previous), owner)) &&
        (await fs.pathExists(onDisk(previous)))
      ) {
        current.set(name, previous)
        attempted.add(onDisk(previous))
      }
    }
    // Only a plain file can be renamed this pass. A failed hashed Sass entry
    // may already have carried its last-good output into current above.
  ...
  for (const [name, { relative, plain, sources }] of claimed) {
    ...
    if (hash && isHashable(relative)) {
      ...
      outputs?.release(plain, owner)
    }
    outputs?.claim(onDisk(emittedName), owner, 'asset')
    attempted.add(onDisk(emittedName))
    if (emittedName !== name) attempted.delete(plain)
  ...
  for (const stale of manifest.reconcile(owner, current)) {
    ...
    if (!outputs || outputs.owns(file, owner))
      await fs.unlink(file).catch((error) => {
        if (error.code !== 'ENOENT') throw error
      })
    if (!attempted.has(file)) outputs?.release(file, owner)
  }
  outputs?.retain(owner, attempted)
}
...
export async function copyAssets(
  ...
  owner ??= assetCopyOwner(sourceDir, targetDir)
  ...
  const sass = await Promise.all(
    compileSassFiles(sourceDir, targetDir, { config, logger, outputs, owner }),
  )
  const refused = new Set(
    sass.filter((item) => item.refused).map((item) => item.file),
  )
  try {
    ...
    if ((await fs.pathExists(sourceDir)) || !manifest.hasOwner(owner))
      await fs.copy(sourceDir, targetDir, {
        filter: (src, dest) => {
          if (isSass(src)) return false
          if (!outputs || outputs.canWriteAsset(dest, owner)) return true
          refused.add(path.relative(sourceDir, src))
          return false
        },
      })
    ...
    // Fingerprint compiled bytes in memory, not the plain output path which
    // hashing has moved. A copied CSS sibling wins and is not a Sass change.
    const fingerprints = new Map()
    for (const item of sass) {
      if (!item.fingerprint) continue
      const css = item.file.replace(/\.(scss|sass)$/i, '.css')
      if (!(await fs.pathExists(path.join(sourceDir, css))))
        fingerprints.set(item.file, item.fingerprint)
    }
    const changed = manifest.reconcileSass(owner, fingerprints)
    for (const item of sass) item.changed = changed.has(item.file)
    const msg = `Copied assets: ${shownSource} to ${shownTarget}`
    logger.info(msg)
    return { id, data: msg, sass, refused: [...refused] }
  } catch (err) {
