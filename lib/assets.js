...
export function compileSassFiles(sourceDir, targetDir, { config, logger }) {
  ...
  return (
    globFiles(root, '**/*.+(scss|sass)')
      // A leading underscore is THE Sass convention for "this is a partial, do
      // not compile me standalone", and dart-sass itself never compiles one: a
      // partial is meant to be `@use`d by an entry point that defines the
      // variables it reads. kiss globbed them anyway, which was a harmless red
      // log line right up until a Sass error became a build failure — and then
      // the most ordinary stylesheet layout there is could not build, while the
      // real stylesheet compiled correctly beside it.
      .filter((sassFile) => !path.posix.basename(sassFile).startsWith('_'))
      .map(async (sassFile) => {
