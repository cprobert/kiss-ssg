...
async function recordEmitted(
  ...
  for (const file of globFiles(root, '**/*')) {
    ...
    if (previous) {
      ...
      // runs first and `fs.copy` writes over the top of it, so the plain file
      // is always the one served and the Sass is always the one silently
      // discarded. Naming them the wrong way round would send the author to
      // edit the file that is already winning.
      const [loser, winner] = isSass(source)
        ? [source, previous]
        : [previous, source]
      logger?.warn(
        `Two assets emit ${name}: ${loser} is compiled there and ${winner} is copied over the top. Only ${winner} is served, so edits to ${loser} do nothing.`,
      )
