import fs from 'fs-extra'
import path from 'node:path'
import { createHash } from 'node:crypto'
// html-minifier-terser costs ~155ms to load (it pulls in terser and a CSS
// parser) and `lib/kiss.js` imports this module eagerly for the `KissPage`
// class, so as a static import the cost landed on every process that touched
// Kiss — including ones that render nothing. It is loaded on demand instead.
//
// A production build cannot avoid it, though: every page is minified. Paid at
// the first render it sits squarely on the critical path, serial with the
// work, and showed up as ~+95ms in the `build` phase of a 50-page site. So
// `Kiss` calls `preloadMinifier()` from its constructor when it is not in dev
// mode, and the load overlaps folder setup, asset copying, partial registration
// and model resolution instead. Dev never minifies and never pays it.
//
// The *promise* is cached, not the resolved function: every page's render
// starts concurrently, and caching the result would have each of them call
// `import()` before the first had resolved. Node dedupes the load, so that was
// harmless, but one shared promise is the honest shape.
// https://www.npmjs.com/package/html-minifier-terser
/** @type {Promise<Function>|null} */
let minifierLoad = null
export const loadMinifier = () =>
  (minifierLoad ??= import('html-minifier-terser').then((m) => m.minify))

/**
 * Starts loading the HTML minifier without waiting for it, so its import cost
 * overlaps the work between construction and the first render. A load failure
 * is swallowed here and resurfaces from `loadMinifier()` at the render that
 * needs it, where it is reported against a page.
 */
export const preloadMinifier = () => {
  loadMinifier().catch(() => {})
}
import { toSlug, toTitleCase, sanitizePath } from './utils.js'
import { extractReferences } from './links.js'
import { createLogger } from './logger.js'
import { DEFAULT_CONFIG } from './config.js'

// Compiled templates, keyed per Handlebars environment.
//
// `_getTemplate` runs on every render, and used to read and compile the view
// each time. A one-shot `scan` build hides that — one page, one view, one
// compile — but a fan-out does not: `.pages({ view: 'item.hbs', model: 'items' })`
// renders N pages from ONE template and was compiling it N times. On a real
// site (diploma-msc) 111 course pages share four `catalogs/*.hbs` views, so
// each was compiled ~28 times. Handlebars compilation is 44% of a 500-page
// build in the CPU profile, and it is the compiler, not the renderer.
//
// A WeakMap on the environment, not a plain module-level Map, because
// `hbs.compile` closes over the environment it was called on: its helpers and
// partials are resolved against that instance. Two Kiss instances in one
// process have two environments, and a template compiled on one must never be
// handed to the other. Keying on the env also lets the cache die with it.
//
// An entry is validated by *content*, not mtime: the view is read on every
// render and compiled only when its text differs from the text last compiled.
// Reading a template costs ~0.03ms; compiling it costs milliseconds, and the
// compile is the whole saving. An mtime check was cheaper still but depended on
// the filesystem's timestamp granularity — exFAT and some network and WSL2
// mounts round to a second or two, and two saves inside one tick after a render
// would have served the stale template until the next save. Comparing the text
// has no such dependency and needs no `stat`.
const templateCache = new WeakMap()

export class KissPage {
  /** @private @type {string} */
  _directory
  /** @private @type {string|null} */
  _outputPath = null
  /** @private @type {string} */
  _buildDir = './public'
  _path = ''
  _slug = 'index'
  _ext = 'html'
  _extLess = false
  _buildTo = ''
  _title = 'Kiss page'
  _dev = false

  _debug = false
  view = null
  options = {}

  // sha1 of the bytes this page last wrote — the identity of the output, not of
  // the inputs that produced it, so two builds whose templates and models were
  // rewritten into the same HTML compare equal. `null` until a write succeeds,
  // and `null` again after one that did not: a page that failed, or one with
  // `generate: false`, has no bytes on disk to name. `BuildPage.hash` carries
  // it, and `diffReports` (lib/check.js) is what reads it back.
  /** @type {string|null} */
  hash = null

  // Every reference the bytes this page last wrote ask a browser to fetch, as
  // written — the raw `href`/`src`/`srcset`/`action` values, deduplicated and
  // sorted. Extracted here rather than by reading the file back, because under
  // `kiss-ssg check` the folder is gone before anything could read it; a few
  // hundred bytes a page, never the HTML itself. `null` exactly when `hash` is:
  // no bytes on disk, nothing to have referenced. `lib/links.js` resolves them.
  /** @type {string[]|null} */
  links = null

  // defaults
  get buildDir() {
    return this._buildDir
  }
  set buildDir(value) {
    this._buildDir = value
    this._outputPath = null
  }
  pagesDir = './src/pages'
  // Annotated, or the frozen default narrows the field to the literal 35729
  // and any real port assigned from config fails to typecheck.
  /** @type {number} */
  livereloadPort = DEFAULT_CONFIG.livereloadPort

  /**
   * @param {string} view a `.hbs` filename under `pagesDir`, or inline template
   *   source
   * @param {{ hbs?: any, logger?: any, directory?: string, graph?: import('./dependency-graph.js').DependencyGraph, outputs?: import('./output-registry.js').OutputRegistry }} [deps]
   */
  constructor(
    view,
    { hbs, logger, graph, outputs, directory = process.cwd() } = {},
  ) {
    this._directory = directory
    this.view = view
    this.hbs = hbs
    this.logger = logger || createLogger()
    this.graph = graph
    this.outputs = outputs
    this._title = toTitleCase(this._slug)
  }

  set path(path) {
    if (path) {
      this._path = sanitizePath(path)
      this._outputPath = null
    }
  }

  get slug() {
    return this._slug
  }

  set slug(slug) {
    if (slug) {
      this._slug = toSlug(slug)
      this._outputPath = null
    }
  }

  set ext(extension) {
    if (extension) {
      // Slugified like `path` and `slug`: an unsanitised extension carries
      // `/` and `..` straight into `buildTo` and writes outside the build dir.
      this._ext = toSlug(extension.replace(/^\./, ''))
      this._outputPath = null
    }
  }

  set extLess(val) {
    this._extLess = !!val
    this._outputPath = null
  }

  get buildTo() {
    return `${this.buildDir}/${this.pageURL()}`
  }

  // Filesystem identity is anchored to this registration; buildTo retains
  // the configured spelling for reports and templates.
  get outputPath() {
    return (this._outputPath ??= path.resolve(this._directory, this.buildTo))
  }

  // Inline bodies must not become diagnostic identity in a saved report.
  get outputOwner() {
    return this.view.endsWith('.hbs')
      ? `page: ${this.view}`
      : `page: <inline ${createHash('sha1').update(this.view).digest('hex').slice(0, 12)}>`
  }

  pageURL() {
    // Fake extension less pages
    let pagePath
    if (this._extLess && this.slug !== 'index') {
      pagePath = `${this._path}/${this.slug}/index.${this._ext}`
    } else {
      pagePath = `${this._path}/${this.slug}.${this._ext}`
    }
    if (pagePath.startsWith('/')) pagePath = pagePath.replace(/^\//, '')
    return pagePath
  }

  set isDev(dev) {
    this._dev = !!dev
  }
  set debug(dev) {
    this._debug = !!dev
  }

  prepare() {
    this.options = {
      ...{
        title: this._title,
        path: this._path,
        slug: this._slug,
        generate: true,
      },
      ...this.options,
    }
    return this
  }

  async generate() {
    const template = this._getTemplate(this.view)
    if (template && this.options.generate) {
      // Cleared before the attempt, not after a failure: every path out of the
      // try rethrows, so the one place that can promise "no hash unless these
      // exact bytes are on disk" is here, before anything is written.
      this.hash = null
      this.links = null
      try {
        this.options.pageURL = this.pageURL()
        // The page's identity rides in Handlebars' data frame, which every
        // partial call inherits — nested, dynamic and layout alike — so the
        // recording partials (lib/partials.js) know who is rendering with no
        // shared state. Its old edges go first: a render is the whole truth
        // about what this page uses now.
        this.graph?.clearPage(this.buildTo)
        let output = template(this.options, {
          data: { kissPage: this.buildTo },
        })

        if (this._dev) {
          // The host is resolved in the browser, not baked in here: with
          // devHost '0.0.0.0' the page is opened from another device, whose
          // localhost is not the machine running the livereload server.
          const liveReload = `\n<script>(function () { var s = document.createElement('script'); s.src = 'http://' + (location.hostname || 'localhost') + ':${this.livereloadPort}/livereload.js?snipver=1'; document.body.appendChild(s) })()</script>`
          output = output.replace('</body>', liveReload + '\n</body>')
        }

        // Dev skips minification entirely rather than running it with every
        // option turned off. It used to do the latter, which still paid for
        // the parse — ~1.3ms a page, ~2.7ms with an inlined stylesheet — and,
        // since the minifier is now imported at first render, still paid the
        // ~155ms to load it. A dev build never ships, so the only thing the
        // pass was still doing (dropping comments) is worth less than the time
        // it costs; keeping them is arguably better for reading dev output.
        var minifiedHtml = this._dev
          ? output
          : await (
              await loadMinifier()
            )(output, {
              collapseWhitespace: true,
              conservativeCollapse: false,
              removeComments: true,
              removeEmptyAttributes: true,
              minifyCSS: true,
              minifyJS: true,
            })

        // Belt-and-braces behind the sanitised path/slug/ext setters: whatever
        // composed this URL, a page must never write outside the build folder.
        const resolvedBuildDir = path.resolve(this._directory, this.buildDir)
        // Revalidate at the write boundary too: the safety guard must not
        // trust a cached path if internal route fields were altered directly.
        const resolvedTarget = path.resolve(this._directory, this.buildTo)
        this._outputPath = resolvedTarget
        if (
          resolvedTarget !== resolvedBuildDir &&
          !resolvedTarget.startsWith(resolvedBuildDir + path.sep)
        ) {
          throw new Error(
            `Refusing to write outside the build folder: ${this.buildTo}`,
          )
        }

        try {
          await fs.outputFile(resolvedTarget, minifiedHtml)
          this.outputs?.claim(resolvedTarget, this.outputOwner, 'page')
          // Hashed after the write, and hashing exactly what was written: a
          // hash recorded for bytes that never reached the disk would report a
          // page as unchanged when it is missing.
          this.hash = createHash('sha1').update(minifiedHtml).digest('hex')
          // Beside the hash, and for the same reason: both describe the bytes
          // that reached the disk, and neither may exist for bytes that did not.
          this.links = extractReferences(minifiedHtml)
        } catch (err) {
          this.logger.error(`Error creating ${this.buildTo}`)
          this.logger.error(err)
          throw err
        }

        // The dev-mode debug sibling is an artifact, not the build output —
        // a failure to write it is logged but must never fail the page.
        if (this.options && this._dev) {
          try {
            await fs.outputJson(
              // Only the trailing extension — `this._ext` can occur earlier in
              // the path too (e.g. a folder named `html`).
              resolvedTarget.replace(/\.[^.]+$/, '.json'),
              this.graph
                ? { ...this.options, partials: this.graph.usesOf(this.buildTo) }
                : this.options,
              { spaces: 2 },
            )
            this.outputs?.claim(
              resolvedTarget.replace(/\.[^.]+$/, '.json'),
              this.outputOwner,
              'page',
            )
          } catch (err) {
            this.logger.error(`Error creating ${this.buildTo}`)
            this.logger.error(err)
          }
        }
      } catch (error) {
        this.logger.error(`Error processing view ${this.view}`)
        this.logger.error(error.message)
        if (this._debug) this.logger.debug(error)
        // Rethrow so Kiss can collect the failure: a build that silently
        // resolves with a missing page is worse than a loud one.
        throw error
      }
    } else {
      this.logger.info('Skipping page generation: ', this.view)
    }
    return this.buildTo
  }

  _getTemplate(view) {
    const isFile = view.endsWith('.hbs')
    const viewPath = isFile ? `${this.pagesDir}/${view}` : null
    // An inline template is its own key (and its own content); a file is keyed
    // on its path and validated against the text read this time, so an edit
    // under `watch` recompiles whatever the filesystem says about its mtime.
    const key = isFile ? viewPath : view

    let viewText = view
    if (isFile) {
      try {
        viewText = fs.readFileSync(viewPath, 'utf8')
      } catch (error) {
        this.logger.error('Error reading view: ', viewPath)
        this.logger.error(error.message)
        // A `.hbs` view is a filename, never an inline template: falling
        // through here would compile the filename itself as the page body.
        throw new Error(`Error reading view: ${viewPath}`, { cause: error })
      }
    }

    let cached = templateCache.get(this.hbs)
    if (!cached) templateCache.set(this.hbs, (cached = new Map()))
    const hit = cached.get(key)
    if (hit && hit.source === viewText) return hit.template

    try {
      const template = this.hbs.compile(viewText)
      // Only a compile that succeeded is worth keeping: caching a null would
      // hide the error log from every later render of the same broken view.
      cached.set(key, { template, source: viewText })
      return template
    } catch (error) {
      this.logger.error('Error rendering view: ')
      this.logger.error(error.message)
    }
    return null
  }
}
