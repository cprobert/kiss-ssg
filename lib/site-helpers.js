...
// Whether the entry's SOURCE declares a `registerHelpers` export. Read rather
// than imported, because that is the whole point: the by-name discriminator
// cannot be applied to a module that failed to import, so applying it after
// the import meant every import failure in a guessed folder was downgraded to
// a warning — and a genuine registrar with a typo then warned and let the
// build continue with no helpers, exit 0. Worse than the hazard the rule was
// drawn for, because the author deliberately wrote that file.
//
// Reading first also means kiss never executes a module it has no reason to
// believe is its own: importing IS executing, so "we only call a registrar we
// recognise" was never true while the recognition happened after the import.
//
// Deliberately syntactic and conservative. A file whose registrar is assembled
// at runtime is not recognised and declines, which is the safe direction for a
// folder nobody pointed kiss at; a folder the author NAMED skips this test
// entirely.
const DECLARES_REGISTRAR = [
  /export\s+(?:async\s+)?(?:function|const|let|var)\s+registerHelpers\b/,
  /export\s*\{[^}]*\bregisterHelpers\b[^}]*\}/,
  /exports\.registerHelpers\s*=/,
  /module\.exports\s*=\s*\{[^}]*\bregisterHelpers\b/,
]

function declaresRegistrar(entry) {
  let source
  try {
  ...
  return DECLARES_REGISTRAR.some((pattern) => pattern.test(source))
}
...
export async function loadSiteHelpers(
  ...
  // The by-name discriminator, applied to the source before anything runs.
  if (!required && !declaresRegistrar(entry)) {
    dropPrevious()
    logger.warn(
      `${entry} declares no registerHelpers export — kiss defaulted to this folder rather than being pointed at it, so it was not imported and the build continues without site helpers. Set folders.helpers to the folder that does hold them, or to null if this site has none.`,
    )
