...
export async function loadSiteHelpers(
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        `Could not import ${entry}: ${/** @type {Error} */ (error).message} — kiss defaulted to this folder rather than being pointed at it, so the build continues without site helpers. Set folders.helpers to the folder that does hold them, or to null if this site has none.`,
      )
