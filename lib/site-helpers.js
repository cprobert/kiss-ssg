...
// Which registrar run, if any, the current execution context belongs to.
//
// The wrapper below has to answer "did the REGISTRAR register this name", and
// a plain flag cannot: the wrapper is live across `await register(kiss)`, so
// anything else that registers a helper inside that window is captured too. A
// build script doing `const kiss = new Kiss(); await somethingAsync();
// kiss.handlebars.registerHelper('mine', fn)` lands squarely in it — measured,
// `mine` came back owned by the registrar, and the next helpers save in dev
// then tore it down and did not restore it, because its `prior` was undefined.
// Silent, and the author's own helper simply disappeared.
//
// `AsyncLocalStorage` answers the question properly: the store follows the
// registrar's own async continuations — an `await` inside it, a `.then`, a
// callback it schedules — and nothing else, so ownership means what the name
// says.
const registrarRun = new AsyncLocalStorage()
...
export async function loadSiteHelpers(
  ...
  try {
    ...
    kiss.handlebars.registerHelper = function (name, fn) {
      // Only what THIS registrar registered, on its own async stack. See
      // `registrarRun` above: the wrapper is live across the await, so without
      // this check a build script's own hand-registered helper is claimed as
      // registrar-owned and destroyed by the next reload.
      if (registrarRun.getStore() === token) {
    ...
    try {
      await registrarRun.run(token, () => register(kiss))
    } catch (error) {
      ...
      // `seen`, not every new name in the registry: a name that appeared
      // during the window but was not the registrar's belongs to whoever
      // registered it, and undoing it here would be the same theft the
      // ownership check above exists to prevent.
