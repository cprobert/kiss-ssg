...
class Kiss {
  ...
  page(options, callback) {
    ...
    if (!this._replaying) this._registrations.push({ ...options })
  ...
  complete(callback) {
    return this._drain().then(async () => {
      // Page render/write failures are collected by generate() rather than
      // thrown at the page, so the whole site still builds; they surface
      // here, once, as an AggregateError. The callback does not fire.
  ...
  async _replay() {
    ...
    // the previous build's output. Replays never overlap each other —
    // `_requestReplay()` serialises them.
    ...
    try {
      for (const options of this._registrations) this.page({ ...options })
    } finally {
    ...
    try {
      ...
      for (const file of previous) {
        ...
        await fs.remove(file.replace(/\.[^.]+$/, '.json'))
        this.logger.info('Removed stale output:', file)
      }
    }
  }

  // Coalesce rebuild requests: while a replay is running, remember that another
  // was asked for and run exactly one more when it settles, so a burst of
  // watcher events (save-all, formatter, branch switch) ends with the newest
  // edit on disk instead of a dropped one. Overlapping replays would otherwise
  // interleave: the second one resets `_stack` under the first, whose pending
  // chains then land in it and make the newer pages lose the `buildTo` dedupe.
  _requestReplay() {
    if (this._replayInFlight) {
      this._replayQueued = true
      return this._replayInFlight
    }
    this._replayInFlight = this._replay()
      .catch((err) => {
        this.logger.error('Error rebuilding site', err.message)
        this.logger.warn(err)
      })
      .finally(() => {
        this._replayInFlight = null
        if (this._replayQueued) {
          this._replayQueued = false
          this._requestReplay()
        }
      })
    return this._replayInFlight
  ...
  watch({ entry = process.argv[1] } = {}) {
    ...
    this._watcher = createWatcher({
      ...
      entry,
      rebuildSite,
      // Tracked so a replay waits for it and complete() drains it; the error
      // is already logged inside KissPage.generate().
      rebuildPage: (stackEntry) => {
        this._generating.push(stackEntry.page.generate().catch(() => {}))
      },
      assetsChanged: () =>
        this.copyAssets(this.config.folders.assets, this.config.folders.build),
      logger: this.logger,
