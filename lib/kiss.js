...
class Kiss {
  ...
  _writeRedirects() {
    ...
    this._redirectsRun = writeRedirects(this._stack, {
      ...
      .then(({ status, files, format }) => {
        this._redirectsFormat = format
      ...
      .catch((error) => {
        ...
        this.logger.error(
          `Error writing redirects (config.redirects.format: ${this._redirectsFormat})`,
        )
