...
class Kiss {
  ...
  watch({ entry = process.argv[1] } = {}) {
    ...
    this._watcher = createWatcher({
      ...
      assetsChanged: (changed) => {
        ...
        // copyAssets always resolves and _reload cannot throw, so nothing
        // here is left unhandled.
        this._assetQueue.then(() => this._reload(this._builtAssetPath(changed)))
      },
      logger: this.logger,
    })
    return this
  ...
  _builtAssetPath(changed) {
    ...
    const built = path.resolve(build, path.relative(assets, changed))
    return /\.(scss|sass)$/i.test(built)
      ? built.replace(/\.[^.]+$/, '.css')
      : built
  }
