...
class Kiss {
  ...
  async _discardStaging() {
    ...
    this._buildSettled = true
    await fs
      .remove(this._stagingDir)
      .then(() => this._outputs.clearUnder(this._stagingDir))
      .catch((err) => {
        this.logger.debug(err.stack)
      })
  }
  ...
  generate(callback) {
    ...
    // `_helpersReady` is first and its result is dropped: `data` is the
    ...
    const run = Promise.all([this._helpersReady, ...this._promises])
      .then(async ([, ...data]) => {
        const pending = []
        this._stack.forEach((entry) => {
  ...
  async _replay() {
    ...
    const previousOwners = new Map(
      previous
        .flatMap((file) => [file, file.replace(/\.[^.]+$/, '.json')])
        .map((file) => [
          file,
          this._outputs.kind(file) === 'page'
            ? this._outputs.owner(file)
            : null,
        ]),
    )
    const removePrevious = async (file) => {
      const owner = previousOwners.get(file)
      if (!owner || this._outputs.owner(file) !== owner) return
  ...
  _runRebuildQueue() {
    ...
    this._rebuildInFlight = (async () => {
      if (assets.length) {
        ...
        this.copyAssets(this.config.folders.assets, this.config.folders.build)
        const result = await this._assetQueue
        retryAssets = !!result.refused?.length
        renderAssets =
