...
class Kiss {
  ...
  async _loadHelpers(fresh = false) {
    const { entry, error } = await loadSiteHelpers(
  ...
  watch({ entry = process.argv[1] } = {}) {
    ...
    this._watcher = createWatcher({
      ...
      // An edited helper module re-imports on a cache-busted URL and
      // re-registers — Handlebars replaces a helper of the same name, so
      // there is nothing to tear down. Then every page is re-rendered:
      // helpers only affect rendering, so this is a replay minus its
      // expensive half (no model re-read, no controller re-import, no URL
      // model re-fetch).
      helpersChanged: () => {
