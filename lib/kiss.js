...
class Kiss {
  ...
  copyAssets(sourceDir, targetDir) {
    ...
    // Registered so a replay can re-run this copy. Only a copy that can be
    // re-run is a copy whose failures may be carried — see `_replay()`.
    const copyKey = assetCopyKey(sourceDir, target)
    // Resolved at registration, not stored as written. `_assetCopies` is what
    // a replay re-runs, and a replay happens later — `copyAssets('./vendor',
    // './out')` replayed after something moved the working directory copied a
    // DIFFERENT `./vendor` and keyed it differently, so the original copy's
    // failure was never cleared. Measured. The identity is known here; this is
    // where it gets recorded.
    if (copyKey)
      this._assetCopies.set(copyKey, {
        sourceDir: path.resolve(sourceDir),
        targetDir: path.resolve(targetDir),
      })
    const run = this._assetQueue.then(async () => {
      const result = await copyAssets(sourceDir, target, {
        config: this.config,
        logger: this.logger,
      ...
      if (!sourceDir || !result.sass) return result
      ...
      // is what the author sees in their editor — and absolute when it does
      // not, rather than a `../../..` chain that names nothing recognisable.
      // Display only, now that identity no longer rests on it.
      const relative = utils.posixPath(path.relative(process.cwd(), sourceDir))
      const label = relative.startsWith('..')
        ? utils.posixPath(path.resolve(sourceDir))
        : relative
      ...
      this._refreshReport()
      if (recorded.size) this._sassFailures.set(key, recorded)
      else {
  ...
  robots(options, callback) {
    ...
    const run = Promise.all(this._promises)
      .then(async () => {
        ...
        this._robotsResult = {
          file: `${this.config.folders.build}/robots.txt`,
          agents,
          disallowAll,
          sitemaps,
        }
