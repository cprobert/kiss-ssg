...
class Kiss {
  ...
  // catch, which would mislabel it as "Error generating site". The returned
  // value is never awaited (that is what keeps complete() inside a callback
  // from deadlocking — see _drain), so a promise it returns gets its own
  // catch; a callback that rejects only after complete() has already drained
  // is reported by the next complete(), not that one.
  _runCallback(label, callback, arg) {
    ...
    try {
      ...
      if (result && typeof result.then === 'function') result.catch(record)
    } catch (error) {
      record(error)
    }
  }
