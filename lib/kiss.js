...
import { helpersEntry, loadSiteHelpers } from './site-helpers.js'
  ...
  async _loadHelpers(fresh = false) {
    ...
    const { entry, error, registered } = await loadSiteHelpers(
      this.config.folders.helpers,
      {
        ...
        logger: this.logger,
        fresh,
        previous: this._siteHelperNames,
    ...
    if (!entry && this._siteHelperNames.length) {
      for (const name of this._siteHelperNames)
        this.handlebars.unregisterHelper(name)
      this.logger.notice(
        `The helpers entry is gone: unregistered ${this._siteHelperNames.join(', ')}`,
      )
      this._siteHelperNames = []
    }
    if (registered) this._siteHelperNames = registered
    if (error)
      this._failures.push({
        view: '<site helpers>',
        buildTo: entry,
  ...
  watch({ entry = process.argv[1] } = {}) {
    ...
    this._watcher = createWatcher({
      ...
      helpersChanged: (changed) => {
        const entry = helpersEntry(this.config.folders.helpers)
        if (entry && utils.posixPath(entry) !== utils.posixPath(changed)) {
          this.logger.notice(
            `${changed} is imported by the helpers entry rather than being it: kiss re-imports ${utils.posixPath(entry)} alone, so restart to pick this change up — a rebuild would re-register the helper this file used to define`,
          )
          return
        }
        this._helpersReady = this._loadHelpers(true)
