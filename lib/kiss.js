...
// A whole-site replay clears `_failures` and rebuilds the list as it re-runs
...
//
// `<pipeline…>` is deliberately NOT here: it exists only on a site that has
// configured steps, and that is exactly the site whose replay re-runs them.
const carriedAcrossReplay = (view) =>
  view === '<dev server>' ||
  view === '<site helpers>' ||
  view.startsWith('<sass: ')
...
class Kiss {
  ...
  copyAssets(sourceDir, targetDir) {
    ...
    const run = this._assetQueue.then(async () => {
      ...
      const label = relative.startsWith('..')
        ...
        : relative
      const prefix = `<sass: ${label}/`
      // Cleared by that prefix rather than by the files this copy returned, so
      // a stylesheet that has since been DELETED or renamed loses its entry
      // too: it is absent from the new results, so a per-file clear could
      // never reach it, and `_failures` is otherwise cleared only by
      // `_replay()` — which an asset re-copy is not.
      this._failures = this._failures.filter((f) => !f.view.startsWith(prefix))
  ...
  async _replay() {
    ...
    this._failures = this._failures.filter((failure) =>
      carriedAcrossReplay(failure.view),
    )
