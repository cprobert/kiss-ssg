...
import { isActiveHelpersEntry, loadSiteHelpers } from './site-helpers.js'
...
const assetCopyKey = (sourceDir, targetDir) =>
  sourceDir && targetDir
...
class Kiss {
  ...
  /** @private */
  get _writeRoot() {
    return this._stagingDir ?? this.config.folders.build
  }
  ...
  get _writeConfig() {
    return this._stagingDir
      ? {
          ...this.config,
          folders: { ...this.config.folders, build: this._stagingDir },
        }
      : this.config
  }
  /** @private */
   ...
   */
  _copyAssets(sourceDir, targetDir, watch = false) {
    ...
    const target = this._stagedPath(requested)
    ...
    const run = this._assetQueue.then(async () => {
      const result = await copyAssets(source, target, {
        config: {
          ...this.config,
          folders: {
            ...this.config.folders,
            build: this._stagedPath(this._defaultAssetCopy.targetDir),
          },
        },
        logger: this.logger,
      ...
      // can re-check one.
      const owned = this._sassFailures.get(key)
      if (owned) this._failures = this._failures.filter((f) => !owned.has(f))
      const recorded = new Set()
      for (const { file, error } of result.sass ?? []) {
        if (!error) continue
        const failure = this._carry({
      ...
      this._refreshReport()
      if (owner && this._outputs.hasProducer(owner))
        this._restorableAssets.set(owner, {
          sourceDir: source,
          targetDir: requested,
        })
      else if (owner) this._restorableAssets.delete(owner)
      ...
      if (recorded.size) {
        this._sassFailures.set(key, recorded)
        ...
        this._assetCopies.set(key, { sourceDir: source, targetDir: requested })
      } else {
        this._sassFailures.delete(key)
        ...
        this._assetCopies.delete(key)
      }
      return result
    })
    this._assetQueue = run
    if (!watch) this._promises.push(run)
    return run
  }
  ...
  async _promote() {
    ...
    this._outputs.relocate(staging, target)
  ...
  async _finishBuild() {
    ...
    const verdict = (this._aikbVerdict = await this._buildAikb())
    ...
    this._report = buildReport(this._reportInputs())
    ...
    if (this.config.dev && this.config.verbose)
      await fs
        .outputJson(
          `${this._writeRoot}/dependency-graph.json`,
          this._graph.toJSON(),
          { spaces: 2 },
        )
        .then(() =>
    ...
    if (reportFile)
      ...
      await fs
        ...
        .then(() => this._outputs.claim(reportFile, 'build report'))
        .catch((err) =>
  ...
  _reportInputs() {
    return {
      ...
      feed: this._feedPath,
      robots: this._robotsResult,
      outputs: { collisions: this._outputs.snapshot() },
    }
  }
  ...
  _writeRedirects() {
    ...
    this._redirectsRun = writeRedirects(this._stack, {
      ...
      .then(({ status, files, formats, unset }) => {
        this._redirectsFormats = formats
        ...
        const buildDir = this._writeRoot
        ...
        this._redirectsPath = hostFile ?? null
        this._redirectsJsonPath = files.includes(`${buildDir}/redirects.json`)
      ...
      .catch((error) => {
        ...
        this.logger.error(
          `Error writing redirects (config.redirects.format: ${JSON.stringify(this._redirectsFormats)})`,
        )
        this.logger.warn(error)
        this._failures.push({ view: '<redirects>', buildTo: null, error })
      })
  ...
  _redirectFindings() {
    ...
    const trailingSlash = this.config.links?.trailingSlash ?? true
    const rules = collectAliases(this._stack, { buildDir, trailingSlash })
    ...
    const { removed, collisions, moved } = redirectFindings({
      ...
      // uses.
      currentPages: this._stackForRecord()
    ...
    return {
      ...
      rules,
      json: this._redirectsJsonPath,
      formats: this._redirectsFormats,
      files: this._redirectsFiles,
      removed,
      collisions,
      moved,
    }
  ...
  async _discardStaging() {
    ...
    await fs
      .remove(staging)
      .then(() => this._outputs.clearUnder(staging))
      .catch((err) => {
        this.logger.debug(err.stack)
      })
  }
   ...
   */
  _preparePage(options, origin, idPrefix, directory = process.cwd()) {
    ...
    kissPage.buildDir = this._writeRoot
    kissPage.pagesDir =
      this.config.folders.pages &&
      path.resolve(directory, this.config.folders.pages)
    ...
    kissPage.extLess =
      options.config?.extensionLess ?? this.config.extensionLess
    ...
    if (options.canonical != null && !isAbsoluteHttpUrl(options.canonical)) {
      ...
      this.logger.error(error.message)
      this._failures.push({ view: preparedPage.view, buildTo, error })
      return null
    }
    ...
    if (Array.isArray(data)) {
      for (const model of data) {
        ...
        try {
          ...
          this._preparePage(pageOptions, origin, idPrefix, directory)
        } catch (error) {
  ...
  _page(options, directory) {
    ...
    if (!this._replaying) {
      ...
      this._registrationDirectories.set(registration, directory)
    }
      ...
      .then(async (response) => {
        try {
          if (options.dynamic) {
            ...
            this._preparePage(options, origin, undefined, directory)
          }
        } catch (error) {
  ...
  async _replay() {
    ...
    const removePrevious = async (file) => {
      if (this._outputs.kind(file) !== 'page') return
      const owner = this._outputs.owner(file)
      ...
      this._outputs.release(file, owner)
      for (const asset of this._outputs.assetOwners(file)) restore.add(asset)
      if (removed) this.logger.info('Removed stale output:', file)
    }
    this._stack = []
    this._outputs.beginPages()
    // The stack is gone, so is everything the graph knew about it; every page
    // re-records itself as the replay renders it.
    this._graph.clear()
    this._promises = []
    this._generating = []
    this._failures = this._failures.filter((failure) =>
      this._carriedFailures.has(failure),
    )
    ...
    this._feedPath = null
    this._robotsResult = null
    this._links = null
    this._redirectsPath = null
    this._redirectsJsonPath = null
    this._redirectsFormats = []
    this._redirectsFiles = []
    ...
    try {
      ...
      if (this.config.assets.pipeline.length > 0) {
        ...
        redone.add(
          assetCopyKey(
            this._defaultAssetCopy.sourceDir,
            this._stagedPath(this._defaultAssetCopy.targetDir),
          ),
        )
        this.copyAssets(
          this._defaultAssetCopy.sourceDir,
          this._defaultAssetCopy.targetDir,
        )
      }
      ...
      // avoid.
      for (const [key, owned] of this._sassFailures) {
        if (redone.has(key)) continue
        if (!this._failures.some((failure) => owned.has(failure))) continue
        const copy = this._assetCopies.get(key)
        if (copy) this.copyAssets(copy.sourceDir, copy.targetDir)
      }
      this.registerPartials()
      ...
      try {
        for (const options of registered)
          this._page({ ...options }, this._registrationDirectories.get(options))
      } finally {
      ...
      if (this._robotsRequest) {
        this.robots(this._robotsRequest.options, this._robotsRequest.callback)
      }
      await this.complete()
    } finally {
      ...
      if (!requeued) this._unswept = previous
        ...
        const before = this._assetManifest.urlRevision
        for (const owner of restore) {
          const copy = this._restorableAssets.get(owner)
          if (copy) await this._copyAssets(copy.sourceDir, copy.targetDir, true)
        }
        if (
          this.config.assets.hash &&
          before !== this._assetManifest.urlRevision
        )
          await this._rebuild(this._stack)
        if (restore.size) await this._checkLinks()
      }
      this._refreshReport()
    }
  }
  ...
  _runRebuildQueue() {
    ...
    const assets = [...this._pendingAssets]
    ...
    this._rebuildInFlight = (async () => {
      if (assets.length) {
        await Promise.allSettled([...this._promises, ...this._generating])
        const before = this._assetManifest.urlRevision
        const result = await this._copyAssets(
          this._defaultAssetCopy.sourceDir,
          this._defaultAssetCopy.targetDir,
          true,
        )
        renderAssets =
          this.config.assets.hash && before !== this._assetManifest.urlRevision
        if (
          !replay &&
          !renderAssets &&
          targets.length === 0 &&
          ...
          if (/\.(scss|sass)$/i.test(changed)) {
            ...
            if (
              ...
              refreshPath = null
            if (styles.length === 1)
              refreshPath = this._builtAssetPath(
                path.join(this._defaultAssetCopy.sourceDir, styles[0].file),
              )
          } else refreshPath = this._builtAssetPath(changed)
        }
      }
      if (replay) {
        await this._replay()
      } else if (renderAssets || targets.length)
        await this._rebuild(renderAssets ? this._stack : targets)
    })()
      .catch((err) => {
      ...
      .finally(() => {
        ...
        if (refreshPath !== null) this._reload(refreshPath)
  ...
  async _rebuild(entries) {
    ...
    for (const entry of live) this._clearPageFailure(entry)
  ...
  _handleChange(event, changedPath) {
    ...
    const inPages = inside(this.config.folders.pages)
    ...
    if (matches.length === 0) {
      ...
      if (
        ...
        !inside(this.config.folders.models) &&
        !inside(this.config.folders.controllers) &&
        ...
        !inside(this.config.folders.helpers)
      )
        this.logger.notice(
  ...
  watch({ entry = process.argv[1] } = {}) {
    ...
    this._watcher = createWatcher({
      ...
      helpersChanged: (changed) => {
        ...
        if (isActiveHelpersEntry(this.config.folders.helpers, changed)) {
          this._helpersReady = this._helpersReady.then(() =>
            this._loadHelpers(true),
          )
          this._helpersReady.then(() => this._requestRebuild(this._stack))
          return
        }
      ...
      assetsChanged: (changed, event) => {
        this._pendingAssets.set(changed, event)
        this._runRebuildQueue()
      },
      logger: this.logger,
    })
