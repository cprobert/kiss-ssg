...
class Kiss {
    ...
    const run = this._assetQueue.then(async () => {
      const result = await copyAssets(source, target, {
        ...
        protectedPaths: new Set(
          this._stack.flatMap((entry) =>
            entry.buildTo
              ? [
                  path.resolve(entry.buildTo),
                  path.resolve(entry.buildTo.replace(/\.[^.]+$/, '.json')),
                ]
              : [],
          ),
        ),
  ...
  _runRebuildQueue() {
    ...
    const renderAssets = assets.length > 0 && this.config.assets.hash
    const refreshPath =
      !replay &&
      !renderAssets &&
      targets.length === 0 &&
      assets.length === 1 &&
      assets[0][1] === 'change' &&
      !/\.(scss|sass)$/i.test(assets[0][0])
        ? this._builtAssetPath(assets[0][0])
        : '/'
