...
class Kiss {
  ...
  _copyAssets(sourceDir, targetDir, watch = false) {
    ...
    const run = this._assetQueue.then(async () => {
      const result = await copyAssets(source, target, {
        ...
        owner: 'asset copy: ' + JSON.stringify([source, requested]),
  ...
  _preparePage(options, origin, idPrefix, directory = process.cwd()) {
    ...
    const claimed = this._stack.some(
      (entry) =>
        entry.page.outputPath === preparedPage.outputPath && writes(entry.page),
    )
  ...
  async _replay() {
    ...
    try {
      ...
      if (this.config.assets.pipeline.length > 0) {
        ...
        redone.add(
          assetCopyKey(
            this.config.folders.assets,
            this._stagedPath(this.config.folders.build),
          ),
        )
        this.copyAssets(this.config.folders.assets, this.config.folders.build)
      }
    ...
    this._rebuildInFlight = (async () => {
      if (assets.length) {
        ...
        const result = await this._copyAssets(
          this.config.folders.assets,
          this.config.folders.build,
          true,
        )
        refused = (result.refused ?? []).map((file) =>
          path.resolve(
            this.config.folders.build,
            file.replace(/\.(scss|sass)$/i, '.css'),
          ),
        )
        renderAssets =
          ...
          if (/\.(scss|sass)$/i.test(changed)) {
            ...
            if (styles.length === 1)
              refreshPath = this._builtAssetPath(
                path.join(this.config.folders.assets, styles[0].file),
              )
      ...
      if (replay) {
        await this._replay()
        // A deleted page held its claim until the replay's orphan sweep.
        // Retry once in this batch, then refresh only after the copy settles.
        if (
          refused.some(
            (file) =>
              !this._outputs.kind(file) || this._outputs.kind(file) === 'asset',
          )
        ) {
          const before = this._assetManifest.urlRevision
