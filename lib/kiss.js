...
class Kiss {
  ...
  copyAssets(sourceDir, targetDir) {
    ...
    const run = this._assetQueue.then(async () => {
      ...
      //
      // Every file this copy tried is cleared first, so a stylesheet that has
      // since been fixed loses its entry on a watch rebuild: `_failures` is
      // otherwise cleared only by `_replay()`, and an asset re-copy is not one.
      const views = new Set(
        (result.sass ?? []).map(({ file }) => `<sass: ${file}>`),
      )
      if (views.size)
        this._failures = this._failures.filter((f) => !views.has(f.view))
