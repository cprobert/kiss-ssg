...
import { buildSiteMap, pageOrigin, writeAikb, writeLastBuild } from './aikb.js'
...
class Kiss {
  ...
  async _finishBuild() {
    ...
    const { verdict, map } = await this._buildAikb()
    ...
    // Last of the three files, because it is this build's report and the report
    // did not exist until a moment ago. A check wrote nothing, so it writes
    // nothing here either.
    if (verdict?.written)
    ...
    // After everything, deliberately: the callback receives the map the build
    // actually wrote, and a throw in it cannot change a verdict this build has
    // already reported. It is still recorded as a build failure, so it surfaces
    // on the next `complete()` rather than vanishing.
    if (map && this._aikbRequest?.callback)
      this._runCallback('aikb', this._aikbRequest.callback, map)
    return this._report
  }

  // Builds the site map, evaluates the note rules and — outside check mode —
  ...
  /** @private */
  async _buildAikb() {
    if (!this._aikbRequest) return { verdict: null, map: null }
    const folder = this.config.folders.aikb
    if (!folder) {
      this.logger.error('Cannot write the AIKB: config.folders.aikb is not set')
      return { verdict: null, map: null }
    }
    ...
    const verdict = await writeAikb({
      map,
      folder,
      logger: this.logger,
      // A check publishes nothing, and the AIKB folder is a publish. The map is
      // still built and the notes still evaluated — that is the verdict the
      // check exists to report.
      write: !this._checkMode,
    })
    return { verdict, map }
  }
    ...
    if (Array.isArray(data)) {
      for (const model of data) {
        ...
        try {
          ...
          this._preparePage(pageOptions, origin)
        } catch (error) {
   ...
   * @returns {this}
   */
  aikb(options, callback) {
    // Remembered the way `_sitemapRequest` and `_llmsRequest` are, so a
    // whole-site watch rebuild re-runs it against its own stack. Nothing is
    // queued here: `_finishBuild()` is what writes, once per settled build.
    this._aikbRequest = { options, callback }
    return this
  }
  ...
  async _replay() {
    ...
    try {
      ...
      // Idempotent — `_replay()` never clears `_aikbRequest`, and the write is
      // driven by `_finishBuild()` rather than by the promise queue — but kept
      // beside the other two so all three replay by the same rule.
      if (this._aikbRequest) {
        this.aikb(this._aikbRequest.options, this._aikbRequest.callback)
      }
      await this.complete()
    } finally {
