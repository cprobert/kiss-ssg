...
| [8 · data-fed site](8-data-fed-site/)               | Exemplar  | A site built from a folder of records, validated in the controller — one record is broken; `.aikb()` commits the site's map | `npm run eg8` · exits 1 · `--dev`                      |
...
regression, not a fix. Example 8 also ships a committed `8-data-fed-site/AIKB/` — the folder
`.aikb()` writes on every build, and one authored note beside it — so you can see what a site's
knowledge base looks like before you add `.aikb()` to a site of your own. Reach for examples 1–6
to answer a narrower question — which call, which option, which helper — rather than as a
