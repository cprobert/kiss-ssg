...
- Each `Kiss` instance has its own Handlebars environment. Register custom helpers on `kiss.handlebars` (as the docs always said), not on the global `handlebars` module. Partials live there too: a helper that reads `require('handlebars').partials` finds nothing in v2 — read `kiss.handlebars.partials`, or drop the helper and use Handlebars' native dynamic partial, `{{> (lookup this "partialName")}}`. Every entry of `kiss.handlebars.partials` is a function — a compiled template that also records which page invoked it — so a helper that renders one by name must accept a function: `const p = kiss.handlebars.partials[name]; typeof p === 'function' ? p(ctx, opts) : kiss.handlebars.compile(p)(ctx)` (it was a string only until the partial's first render anyway).
