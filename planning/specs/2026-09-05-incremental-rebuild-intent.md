...
of the v2 engine review, and the rollout order (its steps 1-2 landed; step 3 is
pending a measurement). This document is the _brief_: the success criteria, the
non-goals, and the three open questions the design does not settle. Neither
replaces the other, and the design should be read first.

Whoever restarts this should re-run `/branch-open` rather than treat the criteria
below as still-current — they were written against the engine as it stood on
2026-09-05, before the asset pipeline, the trailing-slash change and
`config.markdown` landed.
