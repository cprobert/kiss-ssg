...
- **2026-09-05 (W3-9 landed)** — livereload compatibility at `1ce9de9`: snippet host resolved at runtime (verified in built output of examples 2 and 4, which have a `</body>`; example 1 has none and never carried a snippet), `delay: 100`, `extraExts` for svg/webp/avif/ico/woff/woff2; failing-first tests for all three, real-livereload behavioural case (svg refreshes, json does not); suite 315/315 twice; two-port probe: each site's pages carry their own port, no literal localhost. Criteria all still met; the Amendment covers the drift. Left as-is and named: README's `env`-helper example shows a hand-written `localhost:35729` tag (illustrative, not engine output). Decision: **ready to close** — `/branch-close` when the operator asks.
