...
- **It evaluates the note rules**, reported as `aikb.notes` on each report and as four lines in the summary. Every controller file, URL model and pipeline step in the map is a **subject**, and the map records each one's current hash, so two of these four are about a note falling behind its subject rather than being absent:
  ...
  | `moved without redirect:`     | `redirects.moved[]`      | `<from> -> <to> (<id>)`: a page the record and this build share an `id` with, whose output path changed, with no `aliases` entry covering the old path. The fix is the line's own `from` path pasted into that page's `aliases`. Needs a record, and an `id` on both sides — a slug rename changes a fan-out item's default id, so that case reads as a removal instead |
...
- **A message starting `link:` is a `{{link}}` that could not be resolved**, and it is a real page failure — the one helper that fails a build rather than degrading. `link: no page with id "<id>" (asked by <view> / <served path>)` names the id that resolved to nothing and the page that asked for it; `link: id "<id>" is the default id of more than one page (<views>), so no page claims it — set an explicit id (asked by …)` is the other form. Three fixes, in the order to check them: the id is wrong (the `Id` column of `AIKB/site-map.md` lists every valid one), the page it names is registered `generate: false` and so claims no id at all, or two pages arrived at the same default id — set an explicit `id` on each. The failing page is the one that wrote the link, not the one it points at.
- **Fix the `failures` entries before the `broken link:` lines.** The scan only knows pages that wrote bytes, so a page that failed makes every `{{link}}` to it a `broken link:` finding on every page that linked it — one failure can fill the summary with link lines that all disappear when it is fixed.
