...
- **A page you rename or move keeps its old URL.** Put the old path in that page's `aliases` (`aliases: ['/old-slug.html']`, on the fan-out **record** rather than the registration) and kiss writes the redirects into the build folder. There is no method to call — an alias is a page option. **Set `redirects: { format: … }` to match the host**: the default `'netlify'` writes `_redirects`, which Netlify and Cloudflare Pages read and **Firebase and Vercel ignore entirely** — on those hosts the default writes a file nothing serves while the old URLs keep 404ing. `'firebase'` and `'vercel'` write a fragment to merge into the host's own config, `'none'` opts out, and a function writes anything else. `report().redirects.rules` and `<build>/redirects.json` carry the list as data whatever you choose.
