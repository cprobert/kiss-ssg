...
- **Contract** — ask only when more than one workstream is expected. Name the `planning/plans/<date>-<slug>.md` the workstreams build against: the one document that says what each workstream owns, which files it may touch, and where the seams are. It is amended **in place**, with a dated section, when the design moves — never forked into a second plan, or the workstreams start building against different documents without anyone noticing. Write the file (or confirm it exists) before the first workstream starts, and put its path in the session file so the reflection can be read against it. **Then critique it before a line is built against it** — spend one fresh-context agent on an adversarial read of the contract alone, asking of every artefact it names: what writes this, when, and what moves its baseline? A contract's load-bearing flaws are findable from the document by someone who has not been in the conversation that produced it, and they are an order of magnitude cheaper to find there than in the workstreams. This has paid twice (`2026-09-10`, where the first design's baseline flaw was findable from the contract alone and was not found; `2026-09-12`, where the critique ran and fifteen blockers came out before any code).
