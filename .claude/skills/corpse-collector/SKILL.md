...
- **Check 5 — npm scripts:** `npm run x` in the docs with no `x` in `package.json`'s `scripts`. `llms.txt` is not read: it ships inside a consumer's `node_modules`, and every `npm run` in it is a script it tells _that_ project to add.
