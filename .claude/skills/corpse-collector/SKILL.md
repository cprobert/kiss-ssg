...
- **Deliberate absence notes** — `AIKB/testing.md` says `lib/kiss.js` has no `test/unit/kiss.test.js` (it's covered by `test/integration/`). Check 1 flags that path as missing on disk, and it is *supposed* to be missing. Same class: "v1 did X", "replaced by Y", "this was removed in v2".
