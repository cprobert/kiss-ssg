...
// ── AIKB doc staleness, by git history ───────────────────────────────────────
// CLAUDE.md's rule is "read the relevant AIKB doc before changing that module,
// and update it in the same commit". That rule is unenforceable by a test —
// `test/aikb.test.js` can only see that a doc exists, not that it is still
// true — so the closest mechanical proxy is history: how many commits have
// touched `lib/<m>.js` since the last commit that touched `AIKB/<m>.md`.
//
// Exported (and given a root) so `test/unit/corpse-collector.test.js` can drive
// it over a throwaway repository instead of this one, whose history changes
// under the test every time someone commits.
export function aikbStaleness(root, { minCommits = 2 } = {}) {
  const git = (args) => {
    try {
  ...
  let modules
  try {
    modules = readdirSync(join(root, 'lib')).filter((f) => f.endsWith('.js'))
  } catch {
    return []
  }
  const rows = []
  for (const file of modules.sort()) {
    const doc = `AIKB/${file.replace(/\.js$/, '')}.md`
    if (!existsSync(join(root, doc))) continue
    // A doc that git has never seen (untracked, or not committed yet) has no
    // "last changed" to count from — and a *missing* doc is already Check 1's
    // and `test/aikb.test.js`'s business, not this one's.
    const last = git(['log', '-1', '--format=%H%x09%h%x09%as', '--', doc])
    if (!last) continue
    const [sha, short, date] = last.split('\t')
    const behind = Number(
      git(['rev-list', '--count', `${sha}..HEAD`, '--', `lib/${file}`]),
    )
    if (!Number.isFinite(behind) || behind < minCommits) continue
    rows.push({
      module: `lib/${file}`,
      doc,
      behind,
  ...
  // Worst first: the row to read before trusting a doc is the one furthest behind.
  return rows.sort(
    (a, b) => b.behind - a.behind || a.module.localeCompare(b.module),
  )
}
...
function main() {
  ...
  function* walk(rel, exts) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        e.isFile() &&
        (!exts || exts.some((x) => e.name.endsWith(x)))
      ) {
        yield child
      }
    }
  ...
  const kissMethods = new Set(
    [...kissSrc.matchAll(/^ {2}(?:async )?([a-zA-Z_]\w*)\(/gm)].map(
      (m) => m[1],
    ),
  )
  const apiRefs = [
  ...
  // Advisory like every other check, and the softest of them: a doc can be
  // several commits behind and still be entirely true. The row means "read this
  // before trusting it", not "this is wrong".
  section(
    'Check 8 — AIKB docs behind their modules',
    aikbStaleness(ROOT),
    (h) =>
      `  ${h.doc}: ${h.behind} commits behind ${h.module} ` +
      `(doc last changed ${h.docSha} ${h.docDate})`,
  )
